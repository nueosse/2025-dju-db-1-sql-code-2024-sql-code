...
DBA의 주요 역할은 데이터베이스의 성능을 모니터링하고, 백업 및 복구 절차를 관리하며, 보안 및 권한을 부여하고, 데이터베이스의 일관성과 무결성을 유지하는 것입니다.

**17 DBMS 의 3가지 필수 기능에 대해 설명하시오.**
정의기능은 데이터베이스 구조를 정의하고 수정하는 기능, 조작기능은 데이터베이스에 대한 데이터 추가, 수정, 삭제 및 검색을 담당하는 기능, 제어기능은 데이터베이스의 접근 권한을 제어하고 보안을 관리하는 기능을 합니다.

**18 데이터 모델에 따른 DBMS 의 역사에 대해 간략히 설명하시오.**
DBMS는 데이터 모델에 따라 발전하여 1세대는 계층형 모델과 망형 모델, 2세대는 관계형 모델을 중심으로 발전하였습니다. 최근에는 객체 지향 모델과 객체-관계형 모델이 등장했습니다.

**19 데이터 사전(data dictionary)과 데이터 디렉토리 (data directory)의 차이점을 설명하시오.**
데이터 사전은 데이터베이스 구조와 정의된 정보를 포함하며 메타데이터를 저장합니다. 반면, 데이터 디렉토리는 데이터베이스의 데이터 파일에 대한 정보를 포함하는 저장소입니다.

**20 다양한 수준의 추상화와 테이터 독립성율 제공하기 위하여 대이터베이스 관리 시스템 (DBMS)에서 지원하는 3 단계 스키마 구조에 대해셔 간략히 설명하시오.**
DBMS는 외부 스키마, 개념 스키마, 내부 스키마라는 3단계 스키마 구조를 통해 다양한 수준의 데이터 추상화 및 독립성을 제공합니다. 외부 스키마는 사용자 관점을, 개념 스키마는 전체 데이터 구조를, 내부 스키마는 물리적 저장 구조를 나타냅니다.
